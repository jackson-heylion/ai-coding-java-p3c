...
4. Identify correctness constraints: contract/presence, auth/data scope, transaction, idempotency/concurrency, nullability, compatibility, and dependency mediation when build files change.
...
7. Review the final diff for accidental contract/security/data/concurrency/dependency changes.
...
- Maven/dependency/plugin/BOM changes → `.ai/rules/maven.md`
...
- For dependency changes, inspect the affected dependency graph only when transitive/version mediation matters.
